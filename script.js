...
decks.forEach((audio, index) => {
  ...
  audio.addEventListener("waiting", () => {
    ...
    if (isActiveDeck(index)) setNowStatus("Conectando");
  });

  audio.addEventListener("playing", () => {
    logLocalDeckEvent(index, "playing", audio);
    if (isActiveDeck(index)) setNowStatus("Reproduciendo");
  ...
  audio.addEventListener("error", () => {
    ...
    if (audio._localPreparing || audio._streamPreparing) return;
    if (!state.nowPlaying || !isActiveDeck(index) || !audio.currentSrc) return;
    if (!isInternalAudioSource(state.nowPlaying)) return;
    if (state.nowPlaying.source === "local") {
      handleLocalPlaybackFailure(state.nowPlaying, audio, new Error(mediaErrorMessage(audio)));
      return;
    }
    handleStreamPlaybackFailure(state.nowPlaying, audio, new Error(mediaErrorMessage(audio)));
  });
});
...
function bindEvents() {
  ...
  dom.nextBtn.addEventListener("click", () => advance(true));
...
async function activateAudio() {
  ...
  await Promise.race([
    Promise.allSettled([...decks, auxiliaryPlayer].map(primeAudioElement)),
    delay(700),
  ]).catch(() => {});
...
function togglePlayPause() {
  ...
  if (active.paused) {
    if (state.nowPlaying.source === "local") ensureAudibleLocalOutput(active);
    active
      .play()
      .then(() => {
        setNowError("");
...
function restartCurrent() {
  ...
  if (state.nowPlaying.source === "youtube" && state.youtube.player?.seekTo) {
  ...
  if (isInternalAudioSource(state.nowPlaying)) {
    ...
    active.play().catch((error) => handlePlayRejection(error));
  }
}

async function advance(useCrossfade) {
  if (!state.audioActivated) {
    showActivationRequired();
    return;
  }
  ...
  try {
    ...
    await playItem(next, useCrossfade);
  } finally {
...
async function playItem(item, useCrossfade) {
  ...
  await playInternalAudioItem(item, useCrossfade);
}

async function playInternalAudioItem(item, useCrossfade) {
  ...
  const hadInternalAudio = state.nowPlaying && isInternalAudioSource(state.nowPlaying);
  const newDeck = hadInternalAudio ? 1 - state.activeDeck : state.activeDeck;
  const nextAudio = decks[newDeck];
  const currentAudio = decks[oldDeck];
  const shouldCrossfade =
    Boolean(useCrossfade && hadInternalAudio && state.settings.crossfadeSeconds > 0) &&
  ...
  try {
    if (item.source === "local") {
      ...
      await Promise.all([readyPromise, playPromise]);
    } else {
    ...
    if (state.nowPlaying !== item || state.activeDeck !== newDeck) return;
    setNowError("");
    setNowStatus("Reproduciendo");
  ...
  if (shouldCrossfade) {
    fadeBetween(currentAudio, nextAudio, state.settings.crossfadeSeconds, () => {
      currentAudio.pause();
...
function fadeBetween(fromAudio, toAudio, seconds, done) {
...
function applyMainDucking() {
  ...
  if (state.nowPlaying.source === "youtube" && state.auxiliaryPlayback.previousYoutubeVolume === null) {
    ...
    state.auxiliaryPlayback.previousInternalVolumes = getMainInternalDecks().map(({ audio, index }) => ({
      index,
...
function restoreMainDucking() {
  ...
  state.auxiliaryPlayback.previousInternalVolumes.forEach(({ index, volume }) => {
    const target = state.muted ? 0 : Math.min(volume, outputVolume());
    fadeAudioVolumeTo(decks[index], target, PAD_RESTORE_MS, token);
  });
...
function getMainInternalDecks() {
  ...
  return Array.from(indexes)
    .map((index) => ({ audio: decks[index], index }))
...
function confirmLocalAudioProgress(item, audio, deckIndex) {
  ...
  window.setTimeout(() => {
    if (state.nowPlaying !== item || state.activeDeck !== deckIndex) return;
...
function syncPlayingClass() {
  const anyPlaying = decks.some((audio) => !audio.paused && !audio.ended);
...
async function previewStream() {
  ...
  audio.addEventListener("waiting", () => setStreamStatus("Conectando..."));
  audio.addEventListener("pause", () => {
...
function startYoutubeProgress() {
  stopYoutubeProgress();
  state.youtube.progressTimer = window.setInterval(updateProgress, 500);
}

function stopYoutubeProgress() {
  window.clearInterval(state.youtube.progressTimer);
...
function renderNowPlaying() {
  const item = state.nowPlaying;
  dom.activeDeck.textContent = state.activeDeck === 0 ? "Player A" : "Player B";
...
function renderTransport() {
  let playing = false;
  if (state.nowPlaying?.source === "youtube") {
    playing = safeYoutubeState() === 1;
  } else if (state.nowPlaying && isInternalAudioSource(state.nowPlaying)) {
    playing = !decks[state.activeDeck].paused;
  }
  ...
  dom.nextBtn.disabled = !state.queue.length && !state.autoMode;
  dom.stopBtn.disabled = !state.nowPlaying;
...
function renderQueue() {
  ...
  dom.queueList.querySelectorAll("[data-queue-play]").forEach((button) => {
    button.addEventListener("click", () => playQueueItem(Number(button.dataset.queuePlay)));
  });
...
function tickVu() {
  const active = decks[state.activeDeck];
  const internalPlaying =
    state.nowPlaying && isInternalAudioSource(state.nowPlaying) && !active.paused && !active.ended;
  const youtubePlaying = state.nowPlaying?.source === "youtube" && safeYoutubeState() === 1;
  const playing = internalPlaying || youtubePlaying;
...
function estimateNextTime() {
  ...
  if (state.nowPlaying?.source === "youtube") {
    ...
    const current = decks[state.activeDeck];
    remaining =
      current && Number.isFinite(current.duration)
...
function estimatedClockForQueue(index) {
  ...
  if (state.nowPlaying?.source !== "stream") {
    if (state.nowPlaying?.source === "youtube") {
      ...
      const current = decks[state.activeDeck];
      if (current && Number.isFinite(current.duration)) {
...
function isActiveDeck(index) {
  return index === state.activeDeck && state.nowPlaying && isInternalAudioSource(state.nowPlaying);
}
